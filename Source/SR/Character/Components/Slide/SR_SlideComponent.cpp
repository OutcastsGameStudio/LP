...
void USR_SlideComponent::BeginPlay() {
  ...
  if (!CharacterMovement || !OwnerCharacter || !MotionController ||
    ...
    UE_LOG(LogTemp, Error,
           TEXT("Failed to load components in USR_DashComponent::BeginPlay()"));
    return;
  }

  OwnerCharacter->FOnSlideInputPressed.AddDynamic(this,
                                                  &USR_SlideComponent::Slide);
  OwnerCharacter->FOnSlideInputReleased.AddDynamic(
      this, &USR_SlideComponent::StopSlide);

  MotionController->OnRootMotionCompleted.AddDynamic(
      this, &USR_SlideComponent::LeaveState);
}

// Called every frame
void USR_SlideComponent::TickComponent(
...
bool USR_SlideComponent::CanInitiateSlide() const {
  if (CharacterMovement->GetLastUpdateVelocity().IsNearlyZero() ||
      CharacterMovement->IsFalling() || bIsCrouching) {
    return false;
  }
...
FVector USR_SlideComponent::UpdateSlideDirection() {
  ...
  if (PerformGroundCheck(HitResult)) {
    ...
    SlideDirection =
        FVector::VectorPlaneProject(FPlayerForward, HitResult.Normal)
            .GetSafeNormal();
  }
...
bool USR_SlideComponent::PerformGroundCheck(FHitResult &OutHitResult) const {
  ...
  return GetWorld()->LineTraceSingleByChannel(OutHitResult, Start, End,
...
void USR_SlideComponent::HandleCrouchFallback() {
  if (CharacterMovement->Velocity.IsNearlyZero() && !bIsCrouching &&
      !CharacterMovement->IsFalling()) {
    bIsCrouching = true;
    CharacterMovement->bWantsToCrouch = true;
    CharacterMovement->Crouch();
  } else if (CharacterMovement->Velocity.IsNearlyZero() && bIsCrouching &&
             !CharacterMovement->IsFalling()) {
    bIsCrouching = false;
...
bool USR_SlideComponent::CheckCollisionAtNewPosition(
    const FVector &NewLocation) const {
  FHitResult HitResult;
  FCollisionQueryParams CollisionParams;
  CollisionParams.AddIgnoredActor(GetOwner());

  return GetWorld()->LineTraceSingleByChannel(
      HitResult, GetOwner()->GetActorLocation(), NewLocation, ECC_Visibility,
      CollisionParams);
}
...
float USR_SlideComponent::GetCurrentFloorAngle() {
  ...
  if (OwnerCharacter->GetWorld()->LineTraceSingleByChannel(
    ...
    FVector ProjectedNormal =
        SurfaceNormal -
        (ForwardVector * FVector::DotProduct(SurfaceNormal, ForwardVector));
    ProjectedNormal.Normalize();

    float SlopeAngle = FMath::RadiansToDegrees(
        FMath::Acos(FVector::DotProduct(SurfaceNormal, FVector(0, 0, 1))));

    float DirectionModifier =
        FVector::DotProduct(ForwardVector, SurfaceNormal) > 0 ? -1.0f : 1.0f;
...
float USR_SlideComponent::CalculateSlideSpeed(float DeltaTime) {
  ...
  float resultGravityForce =
      GravityForce - Friction * Gravity * cos(CurrentFloorAngle);
  ...
  float NewSlideSpeed =
      SlideSpeed + resultGravityForce * DeltaTime * AngleMultiplier;
...
float USR_SlideComponent::ProcessBasicSlide(float DeltaTime) {
  float NewSlideSpeed =
      FMath::Max(0.0f, SlideSpeed - BasicSlideDeceleration * DeltaTime);

  CurrentSlideDistance += SlideSpeed * DeltaTime;

  if (CurrentSlideDistance >= MaxSlideDistance ||
      FMath::IsNearlyZero(NewSlideSpeed, 10.0f)) {
...
void USR_SlideComponent::EnterState(void *data) {
  if (bIsStateActive) {
    UE_LOG(LogTemp, Warning,
           TEXT("USR_SlideComponent::EnterState() - State is already active"));
    return;
  }
...
void USR_SlideComponent::LeaveState(int32 rootMotionId, bool bForced) {
  ...
  if (!ContextStateComponent)
    UE_LOG(
        LogTemp, Error,
        TEXT(
            "Failed to load ContextState in USR_SlideComponent::LeaveState()"));

  if (OwnerCharacter) {
    OwnerCharacter->EnableInput(
        Cast<APlayerController>(OwnerCharacter->GetController()));
  }
