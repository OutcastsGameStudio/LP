...
void USR_DebugComponent::BeginPlay() {
  Super::BeginPlay();
  CharacterMovementComponent = Cast<USR_CharacterMovementComponent>(
...
// Called every frame
void USR_DebugComponent::TickComponent(
...
void USR_DebugComponent::SetWallRunFallingAcceleration(
    float NewWallRunFallingAcceleration) {
  // if(CharacterMovementComponent == nullptr) return;
...
void USR_DebugComponent::InitDataTable() {
  ...
  // 		FSR_DebugData data;
  // 		data.Value =
  // OwnerCharacter->GetCharacterLedgeGrabReachDistance();
  // 		DebugDataTable->AddRow(prop, data);
  // 	} else if(prop ==  "LedgeGrabHeight")
  // 	{
  // 		FSR_DebugData data;
  // 		data.Value = OwnerCharacter->GetLedgeGrabHeight();
  // 		DebugDataTable->AddRow(prop, data);
  // 	} else if (prop == "ClimbUpSpeed")
  // 	{
  // 		FSR_DebugData data;
  // 		data.Value = OwnerCharacter->GetClimbUpSpeed();
  // 		DebugDataTable->AddRow(prop, data);
...
void USR_DebugComponent::SetValuesFromDataTable() {
  ...
  // {
  // 	if (const FSR_DebugData* RowData =
  // DebugDataTable->FindRow<FSR_DebugData>(FName(prop), TEXT("")))
  // 	{
  // 		if (prop == "CharacterLedgeGrabReachDistance")
  // 		{
  // 			OwnerCharacter->SetCharacterLedgeGrabReachDistance(RowData->Value);
  // 		}
  // 		else if (prop == "LedgeGrabHeight")
  // 		{
  // 			OwnerCharacter->SetLedgeGrabHeight(RowData->Value);
  // 		}
  // 		else if (prop == "ClimbUpSpeed")
  // 		{
  // 			OwnerCharacter->SetClimbUpSpeed(RowData->Value);
  // 		}
...
void USR_DebugComponent::UpdateTable(FName RowName, float NewValue) {
  ...
  if (FSR_DebugData *ExistingRow =
          DebugDataTable->FindRow<FSR_DebugData>(RowName, TEXT(""))) {
    // Modifier la valeur
...
FString USR_DebugComponent::GetGameVersion() {
  FString GameVersion;
  GConfig->GetString(TEXT("UserSettings"), TEXT("GameVersion"), GameVersion,
                     GGameIni);
