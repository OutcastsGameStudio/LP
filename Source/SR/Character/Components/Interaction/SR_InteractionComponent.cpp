...
void USR_InteractionComponent::TickComponent(
...
bool USR_InteractionComponent::IsActorInViewAndRange(
    const AActor *Actor, const FVector &CameraLocation,
  ...
  if (!PC->ProjectWorldLocationToScreen(Actor->GetActorLocation(),
                                        ScreenLocation)) {
    return false;
  }
  ...
  FVector2D NormalizedScreenLoc(ScreenLocation.X / ViewportSizeX,
                                ScreenLocation.Y / ViewportSizeY);

  // Calculer la distance par rapport au centre de l'écran (0-1)
  OutDistanceFromCenter =
      FVector2D::Distance(NormalizedScreenLoc, FVector2D(0.5f, 0.5f));
  ...
  bool bIsVisible = !GetWorld()->LineTraceSingleByChannel(
      HitResult, CameraLocation, Actor->GetActorLocation(), ECC_Visibility,
      QueryParams);
...
void USR_InteractionComponent::FindInteractibleActorInView() {
  ...
  UGameplayStatics::GetAllActorsWithTag(GetWorld(), FName("interactible"),
                                        InteractibleActors);
  ...
  for (AActor *Actor : InteractibleActors) {
    ...
    if (IsActorInViewAndRange(Actor, CameraLocation, CameraForward,
                              DistanceFromCenter)) {
  ...
  if (bShowDebug && IsValid(DetectedActor)) {
    DrawDebugLine(GetWorld(), CameraLocation, DetectedActor->GetActorLocation(),
