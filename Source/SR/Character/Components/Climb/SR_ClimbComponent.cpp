...
void USR_ClimbComponent::BeginPlay() {
  ...
  if (!CharacterMovement || !OwnerCharacter || !MotionController ||
    ...
    UE_LOG(
        LogTemp, Error,
        TEXT("Failed to load components in USR_ClimbComponent::BeginPlay()"));
    return;
  }
  OwnerCharacter->OnMoveForwardInputPressed.AddDynamic(
      this, &USR_ClimbComponent::OnMoveForwardInputPressed);
  OwnerCharacter->OnMoveForwardInputReleased.AddDynamic(
      this, &USR_ClimbComponent::OnMoveForwardInputReleased);

  MotionController->OnRootMotionCompleted.AddDynamic(
      this, &USR_ClimbComponent::LeaveState);
}

// Called every frame
void USR_ClimbComponent::TickComponent(
...
bool USR_ClimbComponent::LookAheadQuery() {
  return bIsActive || CharacterMovement->IsMovingOnGround() ||
         ContextStateComponent->GetCurrentMotionState() ==
             MotionState::WALL_RUN;
}
...
void USR_ClimbComponent::UpdateState() {
  ...
  float HorizontalDistance =
      FVector::Distance(HorizontalCurrent, HorizontalTarget);
  ...
  if (bIsAboveLedge && bIsCloseHorizontally) {
    ...
    FVector ForwardImpulse =
        OwnerCharacter->GetActorForwardVector() * ForwardImpulseStrength;
  ...
  // component to maintain height)
    ...
    Direction = FVector(LedgeLocation.X - CurrentLocation.X,
                        LedgeLocation.Y - CurrentLocation.Y, UpwardValue)
                    .GetSafeNormal();
  }
  ...
  FHitResult Hit;
  MovementComp->SafeMoveUpdatedComponent(DesiredMovement,
...
void USR_ClimbComponent::CheckForLedgeGrab() {
  ...
  bool bHitWall = GetWorld()->LineTraceSingleByChannel(
      WallHit, Start, Start + Forward * LedgeGrabReachDistance, ECC_Visibility,
      QueryParams);

  if (bHitWall) {
    FVector EdgeCheckStart =
        WallHit.ImpactPoint +
        FVector(0, 0,
    ...
    FVector EdgeCheckEnd =
        VerticalFrontEnd - FVector(0, 0, LedgeGrabHeight * 3);

    FHitResult EdgeHit; // if we hit a face of the wall
    bool bFoundEdge = GetWorld()->LineTraceSingleByChannel(
        EdgeHit, VerticalFrontEnd, EdgeCheckEnd, ECC_Visibility, QueryParams);

    if (bFoundEdge) // we hit a wall
    {
      auto distanceZFromPlayer = FMath::Abs(
          EdgeHit.ImpactPoint.Z - OwnerCharacter->GetActorLocation().Z);
