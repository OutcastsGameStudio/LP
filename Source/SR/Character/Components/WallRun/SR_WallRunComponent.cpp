...
void USR_WallRunComponent::BeginPlay() {
  ...
  CharacterMovement =
      OwnerCharacter->FindComponentByClass<UCharacterMovementComponent>();
  ContextStateComponent =
      OwnerCharacter->FindComponentByClass<USR_ContextStateComponent>();
  ...
  if (!CharacterMovement || !OwnerCharacter || !MotionController ||
      !ContextStateComponent || !PlayerController) {
    UE_LOG(
        LogTemp, Error,
        TEXT("Failed to load components in USR_WallRunComponent::BeginPlay()"));
    return;
  }

  OwnerCharacter->GetCapsuleComponent()->OnComponentHit.AddDynamic(
      this, &USR_WallRunComponent::OnHit);
  OwnerCharacter->OnMoveForwardInputPressed.AddDynamic(
      this, &USR_WallRunComponent::OnMoveForwardInputPressed);
  OwnerCharacter->OnMoveForwardInputReleased.AddDynamic(
      this, &USR_WallRunComponent::OnMoveForwardInputReleased);

  OwnerCharacter->FOnJumpInputPressed.AddDynamic(
      this, &USR_WallRunComponent::OnJumpButtonPressed);

  MotionController->OnRootMotionCompleted.AddDynamic(
      this, &USR_WallRunComponent::LeaveState);
}

void USR_WallRunComponent::TickComponent(
  ...
  auto velocity =
      FVector(CharacterMovement->Velocity.X, CharacterMovement->Velocity.Y, 0)
          .Size();
  if (bIsStateActive) {
    ...
    if (!DetectNextWall(Hit)) {
      ...
      auto DotProduct =
          FVector::DotProduct(CharacterForwardVector, -Hit.Normal);
...
bool USR_WallRunComponent::HasDetectedPlayerMouseRotation(float DeltaTime) {
  ...
  if (FMath::Abs(CurrentMousePosX - MousePosXAtWallRunStart) >
          MovementThreshold ||
      FMath::Abs(CurrentMousePosY - MousePosYAtWallRunStart) >
          MovementThreshold) {
    return true;
  }
...
void USR_WallRunComponent::UpdateCameraRotation(float DeltaTime) {
  bool hasDetectedPlayerMouseRotation =
      HasDetectedPlayerMouseRotation(DeltaTime);
  if (hasDetectedPlayerMouseRotation) {
    return;
  }

  float TargetRoll = WallRunSide * WallRunCameraRollAngle;
  CurrentCameraRoll = FMath::FInterpTo(CurrentCameraRoll, -TargetRoll,
                                       DeltaTime, CameraRollSpeed);
  ...
  // becomes -180 degrees
  float YawDelta =
      FMath::FindDeltaAngleDegrees(CurrentYaw, TargetYaw) + YawOffset;

  float NewTargetYaw = CurrentYaw + YawDelta;

  float NewYaw =
      FMath::FInterpTo(CurrentYaw, NewTargetYaw, DeltaTime, CameraRollSpeed);

  PlayerController->SetControlRotation(
      FRotator(CurrentRotation.Pitch, NewYaw, CurrentCameraRoll));
}
...
void USR_WallRunComponent::ResetCameraRotation(float DeltaTime) {
  ...
  float Alpha =
      FMath::Clamp(CameraResetTimer / CameraResetDuration, 0.0f, 1.0f);

  CurrentCameraRoll = FMath::Lerp(StartCameraRoll, 0.0f, Alpha);

  FRotator CurrentRotation = PlayerController->GetControlRotation();
  PlayerController->SetControlRotation(
      FRotator(CurrentRotation.Pitch, CurrentRotation.Yaw, CurrentCameraRoll));
...
bool USR_WallRunComponent::DetectNextWall(FHitResult &Hit) {
  ...
  GetWorld()->SweepSingleByProfile(Hit, Start, End, Capsule->GetComponentQuat(),
                                   CollisionProfile,
                                   Capsule->GetCollisionShape(), Params);
  return Hit.bBlockingHit;
}

void USR_WallRunComponent::OnHit(UPrimitiveComponent *HitComponent,
                                 AActor *OtherActor,
                                 UPrimitiveComponent *OtherComp,
                                 FVector NormalImpulse, const FHitResult &Hit) {
  if (!bIsStateActive && bIsMovingForward &&
      FMath::Abs(Hit.Normal.Z) < MaxZThreshold && CheckIfNotInCorner() &&
      CharacterMovement->MovementMode == MOVE_Falling &&
      FVector(CharacterMovement->Velocity.X, CharacterMovement->Velocity.Y, 0)
              .Size() > MinWallRunSpeed) {
    auto CharacterForwardVector = OwnerCharacter->GetActorForwardVector();
    ...
    FVector WallDirection =
        FVector::CrossProduct(FVector::UpVector, Hit.Normal);
    WallRunSide =
        FVector::DotProduct(WallDirection,
                            OwnerCharacter->GetActorForwardVector()) > 0.f
            ? 1
            : -1;
    WallRunDirection = (WallDirection * WallRunSide).GetSafeNormal();

    // enter the wall run state
    USR_ContextStateComponent *ContextState =
        OwnerCharacter->FindComponentByClass<USR_ContextStateComponent>();
    ContextState->TransitionState(MotionState::WALL_RUN);

    // retrieve mouse position
    PlayerController->GetMousePosition(MousePosXAtWallRunStart,
                                       MousePosYAtWallRunStart);
...
void USR_WallRunComponent::EnterState(void *data) {
  if (bIsStateActive) {
    UE_LOG(
        LogTemp, Warning,
        TEXT("USR_WallRunComponent::EnterState() - State is already active"));
    return;
  }
  bIsStateActive = true;
...
bool USR_WallRunComponent::CheckIfNotInCorner() {
  ...
  FVector CameraDirection =
      CameraRotation
          .Vector(); // This gives us the direction the camera is facing
  ...
  bool bHit = GetWorld()->LineTraceSingleByChannel(
      HitResult, OwnerCharacter->GetActorLocation(),
  ...
  bool bHit1 = GetWorld()->LineTraceSingleByChannel(
      HitResult1, OwnerCharacter->GetActorLocation(),
...
void USR_WallRunComponent::UpdateState(float deltaTime) {
  ...
  float WallRunSpeed = FMath::Clamp(CurrentSpeed, MinWallRunClampedSpeed,
                                    MaxWallRunClampedSpeed);
  CharacterMovement->Velocity = WallRunDirection * WallRunSpeed;

  // controlled gravity
  CharacterMovement->Velocity.Z =
      CharacterMovement->Velocity.Z - WallRunFallingSpeed;

  FVector Delta = CharacterMovement->Velocity * deltaTime;
  FHitResult Hit(1.f);

  CharacterMovement->SafeMoveUpdatedComponent(
      Delta, CharacterMovement->UpdatedComponent->GetComponentRotation(), true,
      Hit);
...
void USR_WallRunComponent::DebugLineTrace(FHitResult hitResult, bool hit,
                                          FColor color, FVector vectorStart,
                                          FVector vectorEnd) {
